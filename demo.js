...
function computeDims() {
  PAGE_W = Math.min(660, Math.max(340, window.innerWidth - 80));
  PAGE_H = Math.min(840, Math.max(400, window.innerHeight - 80));
  GCOLS = Math.floor((PAGE_W - MARGIN * 2) / CELL);
  GROWS = Math.floor((PAGE_H - MARGIN * 2) / CELL);
  GOFFX = MARGIN + Math.floor((PAGE_W - MARGIN * 2 - GCOLS * CELL) / 2);
}
computeDims();
var STORY = `There is a mathematics to falling that most people never stop to consider. Not the physics of it, not the acceleration and the inevitable landing, but the geometry — the way a thing in motion seeks its place among other things at rest.

Take the square. It is the most democratic of shapes. It will fit anywhere. It asks nothing of its neighbors. The long straight piece, on the other hand, is particular. It wants open lanes, clear channels, the freedom to slide into places others cannot reach. And the crooked pieces — they have opinions. They arrive already turned, already reaching for something just out of view.

What they all have in common is the falling.

And what falls, eventually, settles. The pieces find each other. Gaps become rows. Rows become history. The space that remains — the space between and above and around what has landed — is where everything else goes. The words, the thoughts, the running commentary of a mind watching shapes descend and asking: what comes next.

There is a kind of patience required. Not the passive kind, but the active kind — the patience of someone who knows that the right piece is coming, and that when it arrives, they will know exactly where it belongs. This is not optimism. It is geometry.

Watch long enough and the falling starts to feel like reading. One thing after another, each finding its line. The white space doing as much work as the shapes themselves. The gaps as meaningful as what fills them.

Every row completed is a small ending. Every cleared line is a beginning in disguise. The board empties upward, not downward, which is the only direction that has ever made any sense at all.`;
var PIECES = [
  { cells: [[0, 0], [1, 0], [2, 0], [3, 0]], color: "#00b4d8" },
  { cells: [[0, 0], [1, 0], [0, 1], [1, 1]], color: "#ffd60a" },
  { cells: [[1, 0], [0, 1], [1, 1], [2, 1]], color: "#9b5de5" },
  { cells: [[0, 0], [1, 0], [1, 1], [2, 1]], color: "#06d6a0" },
  { cells: [[1, 0], [2, 0], [0, 1], [1, 1]], color: "#ef233c" },
  { cells: [[0, 0], [0, 1], [1, 1], [2, 1]], color: "#3a86ff" },
  { cells: [[2, 0], [0, 1], [1, 1], [2, 1]], color: "#fb5607" }
];
function rotateCW(cells) {
  const r = cells.map(([c, row]) => [row, -c]);
  const minC = Math.min(...r.map(([c]) => c));
  const minR = Math.min(...r.map(([, row]) => row));
  return r.map(([c, row]) => [c - minC, row - minR]);
}
function emptyGrid() {
  return Array.from({ length: GROWS }, () => Array(GCOLS).fill(null));
}
var grid = emptyGrid();
var flashRows = [];
var flashStart = 0;
...
function spawn() {
  ...
  const x = Math.floor((GCOLS - 4) / 2);
  if (!canFit(def.cells, x, 0))
    grid = emptyGrid();
  falling = { cells: def.cells.map((c) => [...c]), color: def.color, x, y: 0 };
  lastStep = performance.now();
}
function land() {
  if (!falling)
    return;
  for (const [dc, dr] of falling.cells) {
    const c = falling.x + dc;
    const r = falling.y + dr;
    if (r >= 0 && r < GROWS && c >= 0 && c < GCOLS)
      grid[r][c] = falling.color;
  }
  const full = grid.map((row, i) => row.every((cell) => cell !== null) ? i : -1).filter((i) => i >= 0);
  if (full.length) {
    flashRows = full;
...
function step(now) {
  if (flashRows.length > 0) {
    if (now - flashStart > FLASH_MS) {
      for (const r of [...flashRows].sort((a, b) => b - a)) {
        grid.splice(r, 1);
        grid.unshift(Array(GCOLS).fill(null));
      }
      flashRows = [];
    }
    return;
  ...
  if (now - lastStep < stepMs)
    return;
  lastStep = now;
  if (canFit(falling.cells, falling.x, falling.y + 1))
    falling.y++;
  else
    land();
}
window.addEventListener("keydown", (e) => {
  if (!falling || flashRows.length > 0)
    return;
  if (e.key === "ArrowLeft" && canFit(falling.cells, falling.x - 1, falling.y))
    falling.x--;
  if (e.key === "ArrowRight" && canFit(falling.cells, falling.x + 1, falling.y))
    falling.x++;
  if (e.key === "ArrowUp") {
    const rotated = rotateCW(falling.cells);
    if (canFit(rotated, falling.x, falling.y))
      falling.cells = rotated;
  }
  if (e.key === "ArrowDown")
    stepMs = 80;
  if (e.key === " ") {
    e.preventDefault();
    while (canFit(falling.cells, falling.x, falling.y + 1))
      falling.y++;
    land();
  }
});
...
function blockExclusions(row) {
  if (row < 0 || row >= GROWS)
    return [];
  const spans = [];
  if (flashRows.includes(row)) {
    return [{ left: GOFFX, right: GOFFX + GCOLS * CELL }];
  }
  for (let c = 0;c < GCOLS; c++) {
    if (grid[row][c] !== null) {
      spans.push({ left: GOFFX + c * CELL, right: GOFFX + (c + 1) * CELL });
    }
  }
  if (falling) {
    for (const [dc, dr] of falling.cells) {
...
function subtractSpans(base, excls) {
  let result = [...base];
  for (const e of excls) {
    const next = [];
    for (const s of result) {
      if (e.left >= s.right || e.right <= s.left) {
        next.push(s);
        continue;
      }
      if (e.left > s.left)
        next.push({ left: s.left, right: e.left });
      if (e.right < s.right)
        next.push({ left: e.right, right: s.right });
    }
    result = next;
  }
  return result;
}
var prepared = prepareWithSegments(STORY, FONT);
function layoutLines(px, py) {
  const lines = [];
  let cursor = { segmentIndex: 0, graphemeIndex: 0 };
  let done = false;
  for (let row = 0;row < GROWS && !done; row++) {
    const lineY = GOFFY + row * LINE_H;
    const screenY = py + lineY + (LINE_H - FONT_SIZE) / 2;
    let spans = [{ left: MARGIN, right: PAGE_W - MARGIN }];
    spans = subtractSpans(spans, blockExclusions(row));
    spans = spans.filter((s) => s.right - s.left >= MIN_COL);
    if (spans.length === 0)
      continue;
    for (const span of spans) {
      const result = layoutNextLine(prepared, cursor, span.right - span.left);
      if (result === null) {
        done = true;
        break;
      }
      lines.push({ text: result.text, x: px + span.left, y: screenY });
      cursor = result.end;
    }
  }
  return lines;
...
function pageOrigin() {
  return {
    x: Math.round(Math.max(20, (window.innerWidth - PAGE_W) / 2)),
...
window.addEventListener("resize", () => {
  ...
  resizeTimer = setTimeout(resetGame, 180);
});
function drawBlock(x, y, color, alpha = 1) {
  ...
  ctx.fill();
  const g = ctx.createLinearGradient(x, y, x, y + CELL * 0.6);
...
function drawKey(x, y, label, w = 22) {
  ctx.save();
  ctx.fillStyle = "rgba(30,42,74,0.82)";
  ctx.strokeStyle = "rgba(0,0,0,0.18)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.roundRect(x, y, w, 20, 3);
  ctx.fill();
  ctx.stroke();
...
function drawControls(px, py) {
  ctx.save();
  ctx.globalAlpha = 0.5;
  ctx.textBaseline = "middle";
  const y = py + PAGE_H + 22;
  const cx = px + PAGE_W / 2;
  const gap = 8;
  const sep = 20;
  ctx.font = '11px "Trebuchet MS", sans-serif';
  ctx.textAlign = "left";
  const lbMove = ctx.measureText("move").width;
  const lbRotate = ctx.measureText("rotate").width;
  const lbSpeed = ctx.measureText("speed").width;
  const lbDrop = ctx.measureText("drop").width;
  const spaceW = 54;
  const kw = 22;
  const totalW = kw + 4 + kw + gap + lbMove + sep + kw + gap + lbRotate + sep + kw + gap + lbSpeed + sep + spaceW + gap + lbDrop;
  let x = cx - totalW / 2;
  function group(keys, label) {
    for (let i = 0;i < keys.length; i++)
      drawKey(x + i * (kw + 4), y - 10, keys[i]);
    x += keys.length * kw + (keys.length - 1) * 4 + gap;
    ctx.fillStyle = "rgba(22,33,62,0.9)";
    ctx.fillText(label, x, y);
    x += ctx.measureText(label).width + sep;
  }
  group(["←", "→"], "move");
  group(["↑"], "rotate");
  group(["↓"], "speed");
  drawKey(x, y - 10, "space", spaceW);
  x += spaceW + gap;
  ctx.fillStyle = "rgba(22,33,62,0.9)";
...
function render(now) {
  ...
  ctx.fillStyle = "#ffffff";
  ctx.beginPath();
  ctx.roundRect(px, py, PAGE_W, PAGE_H, 6);
  ctx.fill();
  ctx.strokeStyle = "rgba(0,0,0,0.07)";
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.fillStyle = "rgba(0,0,0,0.015)";
  ctx.fillRect(px + GOFFX, py + GOFFY, GCOLS * CELL, GROWS * CELL);
  ctx.font = FONT;
  ctx.fillStyle = INK;
  ...
  for (let r = 0;r < GROWS; r++) {
    for (let c = 0;c < GCOLS; c++) {
      ...
      const alpha = flashing ? 0.5 + 0.5 * Math.abs(Math.sin((now - flashStart) / FLASH_MS * Math.PI * 3)) : 1;
      drawBlock(px + GOFFX + c * CELL, py + GOFFY + r * CELL, flashing ? "#ffffff" : color, alpha);
    }
  }
  if (falling) {
    let ghostY = falling.y;
    while (canFit(falling.cells, falling.x, ghostY + 1))
      ghostY++;
    if (ghostY > falling.y) {
      ctx.save();
      ...
      for (const [dc, dr] of falling.cells) {
        ctx.beginPath();
        ctx.roundRect(px + GOFFX + (falling.x + dc) * CELL + 2, py + GOFFY + (ghostY + dr) * CELL + 2, CELL - 4, CELL - 4, 3);
        ctx.stroke();
      }
      ctx.restore();
    }
    for (const [dc, dr] of falling.cells) {
      if (falling.y + dr >= 0) {
        drawBlock(px + GOFFX + (falling.x + dc) * CELL, py + GOFFY + (falling.y + dr) * CELL, falling.color);
      }
    }
  }
  drawControls(px, py);
